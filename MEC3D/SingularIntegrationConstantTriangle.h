...
class SingularIntegrationConstantTriangle
{
public:
    SingularIntegrationConstantTriangle(const HammerConstantTriangleIntegration& nonSingIntegration) :
        cNonSingIntegration(nonSingIntegration) {};
    ~SingularIntegrationConstantTriangle() {};

    double IntegrateUStar(const Triangulo& triangulo, const Vetor& ksi,
        const UStarFunNoSing& func, int i, int j, double maxRes = 1e-10) const
    {
        double t = IntegrateUStar(triangulo, ksi, func, i, j, 1);
        ...
        {
            const double t2 = IntegrateUStar(triangulo, ksi, func, i, j, index);
    ...
    double IntegratePStar(const Triangulo& triangulo, const Vetor& ksi,
        const PStarFunNoSing& func, int i, int j, double maxRes = 1e-10) const
    {
        double t = IntegratePStar(triangulo, ksi, func, i, j, 1);

        double res = 0.0;
        int index;
        int indexm1 = 1;

        for (index = 2; index < 1000; index += indexm1)
        {
            indexm1 = index - indexm1;
            //std::cout << index << std::endl;

            const double t2 = IntegratePStar(triangulo, ksi, func, i, j, index);
    ...
    double IntegrateUMindlin(const Triangulo& triangulo, const Vetor& ksi,
        const UStarFunNoSing& func, const FundSolUComp& funcComp, int i, int j, double c = 0.0, double maxRes = 1e-10) const
    {
        double t = IntegrateUStar(triangulo, ksi, func, i, j, 4);
        ...
        for (index = 2; index < 1000; index++)
        {
            double t2 = IntegrateUStar(triangulo, ksi, func, i, j, index);
			
			if (c == 0.0)
			{
				t2 += IntegrateUComp(triangulo, ksi, funcComp, i, j, index);
			}
		...
		if(c!= 0.0)
			t += cNonSingIntegration.Integrate7PtsUStarSubElementComp(triangulo, ksi, funcComp, i, j, c, 0, maxRes);
    ...
    double IntegratePMindlin(const Triangulo& triangulo, const Vetor& ksi,
        const PStarFunNoSing& func, const FundSolPComp& funcComp, int i, int j, double c = 0.0, double maxRes = 1e-10) const
    {
        double t = IntegratePStar(triangulo, ksi, func, i, j, 1);
        ...
        if(c != 0.0)
            t += cNonSingIntegration.Integrate7PtsPStarSubElementComp(triangulo, ksi, funcComp, i, j, c, 0, maxRes);
    ...
    Vetor CoordLocal(const Vetor& no1, const Vetor& no2, const Vetor& no3, const Vetor& pontoFonte, const Vetor& pontoCentral) const
    {
        ...
        const Vetor tmp = CalcInvMatriz(M) * (pontoCentral - pontoFonte);
...
private:
    double IntegrateUStar(const Triangulo& triangulo, const Vetor& ksi,
        const UStarFunNoSing& func, int i, int j, int subDivs) const
    {
        ...
        for (int index = 0; index < subDivs; index++)
        {
            ...
            for (int ni = 0; ni < cPts11.size(); ni++)
            {
                ...
                resv[index] += cWeights11[ni] * Kg * /*(ai * rk * rk / 2.0 * cos(thetai) + bi * rk * rk / 2.0 * sin(thetai) + ci * rk)*/(rk);
            }
    ...
    double IntegratePStar(const Triangulo& triangulo, const Vetor& ksi,
        const PStarFunNoSing& func, int i, int j, int subDivs) const
    {
        std::vector<double> resv;
        resv.resize(subDivs, 0.0);
        ...
        {
            const double a = index * dTheta;
            const double b = (index + 1.0) * dTheta;
            ...
            for (int ni = 0; ni < cPts11.size(); ni++)
            {
                ...
                resv[index] += cWeights11[ni] * Kg * /*(ai * rk * cos(thetai) + bi * rk * sin(thetai) + ci * log(rk))*/log(rk);
            }
    ...
    double IntegrateUComp(const Triangulo& triangulo, const Vetor& ksi,
        const FundSolUComp& func, int i, int j, int subDivs) const
    {
        std::vector<double> resv;
        resv.resize(subDivs, 0.0);
        ...
        for (int index = 0; index < subDivs; index++)
        {
            ...
            for (int ni = 0; ni < cPts11.size(); ni++)
            {
                ...
                const double Kg = func.GetBoussinesqCerrutiSolNonSingPart(i, j, ksi, posEmCoordGlobais, 0.0);
...
private:
    ...
    const HammerConstantTriangleIntegration& cNonSingIntegration;

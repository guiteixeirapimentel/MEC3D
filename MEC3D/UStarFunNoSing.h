...
class UStarFunNoSing
{
public:
	UStarFunNoSing(double poisson, double G) : cCoefElastCis(G), cCoefPoisson(poisson), cCte(1.0 / (16.0 * M_PI * (1.0 - poisson) * cCoefElastCis)) {};
	...
	double operator()(int i, int j, const Vetor& ksi, const Vetor& X) const
	{
		...
		return cCte * (deltakr * (3.0 - 4.0 * cCoefPoisson) + drdi * drdj);
	}
private:
